...
def findPathwayList():
	...
	for code in codes:
		...
		rules=[]
		for i in range(1,6):
			if os.path.isfile('pickles/'+code+'_'+str(i)+'_scores1.pickle'):
				...
				print('pickles/'+code+'_'+str(i)+'_scores1.pickle'+' does not exist')
		...
		for node in range(len(storeModel[1])): 
			ImportanceVals[storeModel[1][node]]=float(np.mean([pathVals[i][node] for i in range(len(pathVals))]))
